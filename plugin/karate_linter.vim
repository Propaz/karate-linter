...
function! s:generate_lint_report()
    ...
    for lnum in range(1, line('$'))
        ...
        if g:karate_linter_unclosed_read_rule && line =~ '^\s*\(Given\|When\|Then\|And\|But\|\*\).*\bread\([^)]*$\)' && line !~ 'header Accept'
...
augroup KarateLinter
  ...
  function! s:setup_karate_lint()
    ...
    if g:karate_linter_unclosed_read_rule
      call add(w:karate_lint_matches, matchadd(g:karate_linter_unclosed_read_level, '^\s*\(Given\|When\|Then\|And\|But\|\*\).*\bread\([^)]*$\)'))
       if !w:karate_has_errors && g:karate_linter_unclosed_read_level == 'KarateLintError'
          if search('^\s*\(Given\|When\|Then\|And\|But\|\*\).*\bread\([^)]*$\)', 'nwc') > 0 | let w:karate_has_errors = 1 | endif
      endif
    endif
